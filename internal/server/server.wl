...
class Server {
    ...
    method take_outgoing() -> Vector(String) {
        let messages -> Vector(String) = self.outgoing;
        self.outgoing = [];
        return messages;
    }

    method __allocate_request_id() -> Int {
        let id -> Int = self.next_request_id;
        self.next_request_id += 1;
        return id;
    }

    method __queue_semantic_refresh() -> Void {
    ...
    method __queue_file_watch() -> Void {
    ...
    method __complete_request(request -> protocol.Request) -> Void {
    ...
    method __read_client_capabilities(params -> protocol.Request) -> Void {
