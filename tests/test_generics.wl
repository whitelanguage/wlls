...
func main() -> Int {
    ...
    if (!has_token(tokens, 0, 17, "typeParameter") || !has_token(tokens, 1, 21, "typeParameter") || !has_token(tokens, 3, 10, "typeParameter")) { print("FAIL: generic type parameter tokens"); return 1; }
    if (!has_token(tokens, 12, 27, "method") || !has_token(tokens, 13, 30, "method")) { print("FAIL: generic member calls"); return 1; }

    workspace.update("project/generic_dep.wl", 1, "class Item<T> {\n    let value -> T;\n    method get() -> T { return self.value; }\n}\nclass Child<T>(Item(T)) {}\n");
